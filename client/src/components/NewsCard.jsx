...
function NewsCard({ title, description, imageUrl, url }) {
  ...
  useEffect(() => {
    ...
    const isAlreadyFavorite = favoriteArticles.some(article => article.title === title && article.url === url);
  ...
  const handleFavoriteToggle = () => {
    ...
    if (!isFavorite) {
      ...
      backendApi.post("favorites", favoriteArticle)
        .then(response => console.log("Saved to favorites:", response.data))
        .catch(error => console.error("Error saving favorite:", error));
    } else {
      let favoriteArticles = JSON.parse(localStorage.getItem("favorites")) || [];
      favoriteArticles = favoriteArticles.filter(article => article.title !== title || article.url !== url);
      localStorage.setItem("favorites", JSON.stringify(favoriteArticles));

      backendApi.delete("favorites", { data: favoriteArticle })
        .then(response => console.log("Removed from favorites:", response.data))
